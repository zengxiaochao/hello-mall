...
<!--        商品数量-->
        <el-table-column label="数量"  prop="num" width="140px" align="center">
          <template slot-scope="scope">
            <el-input-number size="mini" v-model="scope.row.nums"></el-input-number>
          </template>
        </el-table-column>
<!--        商品小计-->
...
<!--        操作-->
        <el-table-column align="right">
          ...
          <template slot-scope="scope">
            <el-button size="mini" type="warning" @click="handleEdit(scope.$index, scope.row)">收藏商品</el-button>
            <el-button size="mini" type="danger" @click="handleDelete(scope.$index, scope.row)">移出购物车</el-button>
            <el-button size="mini" type="success" @click="handleDelete(scope.$index, scope.row)">结算</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
      ...
      <div class="balance">
        <ul class="balance_ul1">
          ...
          <span class="balance_ul2">
              <span>已经选择<span class="spanText">{{allPrices}}</span>件商品</span>
              <span>总价<span class="spanText">&yen;{{allPrices}}</span></span>
              <span>
          ...
          changeNumPri()
          {
              this.$nextTick(() => {
                  ...
                  for (let i = 0; i < length; i++) {
                    allPricess += this.multipleSelection[i].nums*this.multipleSelection[i].price;
                    allNumss += this.multipleSelection[i].nums;
                  }
                  this.allPrices = parseFloat(allPricess).toFixed(2);
                  this.allNums = allPricess;
              });
          },
          initChoose(){
            this.$nextTick(() => {
